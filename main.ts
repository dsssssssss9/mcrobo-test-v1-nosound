...
function show_frown () {
    for (let index22 = 0; index22 <= frown.length - 1; index22++) {
        strip.setPixelColor(frown[index22], neopixel.colors(NeoPixelColors.Green))
        strip.show()
    }
    basic.pause(500)
    strip.clear()
...
function show_smile () {
    for (let index23 = 0; index23 <= smile.length - 1; index23++) {
        strip.setPixelColor(smile[index23], neopixel.colors(NeoPixelColors.Violet))
        strip.show()
    }
    basic.pause(500)
    strip.clear()
